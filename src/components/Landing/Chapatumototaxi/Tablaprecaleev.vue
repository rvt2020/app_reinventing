<template>
  <div>
    ...
    <q-table
      ...
      title="Chapa tu moto taxi Precalificados - Evaluación"
      :data="dataTable"
      dense
      :filter="filter"
      row-key="name"
        ...
        <q-card-section>
          <div class="row">
            <div class="col-xs-12 col-sm-12 q-pa-xs">
              <q-select
                v-model="tcafirma"
                :options="get_tcafirma.resultado"
                option-value="co_afirma"
                option-label="no_afirma"
                emit-value
                map-options
                label="¿Converus?"
                outlined  
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-12 q-pa-xs">
              <q-select
                v-model="tcaprrec"
                :options="get_tcaprrec.resultado"
                option-value="co_aprrec"
                option-label="no_aprrec"
                emit-value
                map-options
                label="Resultado"
                outlined  
    ...
    >
      <q-card style="width: 600px">
        ...
        <q-card-actions align="right">
          <q-btn outline label="Cancelar" color="red" @click="cerrar" />
          <q-btn outline label="Carta de Aprobación" color="green" @click="adddocume" />
        </q-card-actions>
  ...
  methods: {
    ...
    async onSubmit() {
      try {
        const resp = await this.call_update_landin({
          ...
          co_entida: null,
          co_tippla: null,
          il_exacvr: this.tcafirma ? this.tcafirma : null,
          ti_estcvr: this.tcaprrec ? this.tcaprrec : null,
          il_comite: null,
          ti_estcmt: null,
          es_carapr: null,
          fe_activa: null,
    ...
    async gestionar(val) {
      try {
        ...
        await this.call_tcafirma();
        await this.call_tcaprrec();
