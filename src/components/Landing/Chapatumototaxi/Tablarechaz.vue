<template>
  <div>
    ...
    <q-table
      ...
      title="Chapa tu moto taxi Rechazados - Interesados"
      :data="dataTable"
      dense
      :filter="filter"
      row-key="name"
      ...
      <q-card style="width: 600px">
        ...
        <q-card-section>
          <div class="row">
            <div class="col-xs-12 col-sm-12 q-pa-xs">
              <q-select
                v-model="tcdocumerechaz"
                :options="get_tcdocumerechaz.resultado"
  ...
  methods: {
    ...
    async adddocume() {
      try {
        const resp = await this.call_insert_arcadj({
          co_landin: this.clietneSelect.CodLanding,
          ti_docume: this.tcdocumerechaz ? this.tcdocumerechaz : null,
    ...
    async documentar(val) {
      try {
        ...
        this.clietneSelect = val;
        // await this.call_tctiplan();
        await this.call_tcdocumerechaz();
        this.confirm2 = true;
        this.$q.loading.hide();
      } catch (error) {
        console.log(error);
