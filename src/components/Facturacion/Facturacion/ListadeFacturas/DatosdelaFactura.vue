    ...
    <q-card flat bordered>
      <q-markup-table dense>
        <tbody>
          ...
          <tr>
            <td class="text-left">Tipo Documento</td>
          ...
          <tr>
            ...
            <td class="text-right">{{ info.operac[0].im_totfac }}</td>
          </tr>
        </tbody>
      </q-markup-table>
    </q-card>
...
export default {
  props: ["info"],
  name: "DatosdelaFactura",
  methods: {
    ...mapActions("facturas", ["call_update_factur"]),
    async actualizar() {
      await this.call_update_factur({
        co_factur: this.co_factur,
        co_client: this.co_client
      });
    }
  }
};
